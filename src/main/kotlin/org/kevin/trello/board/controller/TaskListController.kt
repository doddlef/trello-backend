...
class TaskListController(
    ...
    fun archiveTaskList(@PathVariable("listId") listId: String): ApiResponse {
        val account = SecurityUtils.currentAccount()
            ?: throw TrelloException("User must be logged in to archive a task list")

        return taskListService.archiveTaskList(listId, account)
    }
}